...
import src.evaluation.eval_prompts as eval_prompts
from src.evaluation.chain_prompts import execute_chain
...
def sentence_simplicity(sentence: str) -> float:
    ...
    if n_root:
        ...
        total_children = [
            len(list(token.children)) for token in doc
        ]  # if token.dep_=='ROOT']
...
def bert_score(input_text: str, output_text: str) -> float:
    ...
    bert = eval.load("bert")
    bert_score = bert.compute(predictions=[output_text], references=[input_text])
...
def bleu(input_text: str, output_text: str) -> float:
    ...
    bleu = eval.load("bleu")
    bleu_score = bleu.compute(predictions=[output_text], references=[input_text])
...
def perplexity(text: str) -> float:
    ...
    score = (
        ...
        if perplexity_score["mean_perplexity"]
        else 0.0
    )
    return float(score)
